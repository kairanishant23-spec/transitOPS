...
const DEFAULT_AVATAR = "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=80&auto=format&fit=crop&q=60";
const DEFAULT_VEHICLE_IMAGE = "https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?w=150&auto=format&fit=crop&q=60";

const ADMIN_ROLES = ["Fleet Manager"];
...
function showLoginPage(type) {
  ...
  updateLoginFormForType(type);
}

function updateLoginFormForType(type) {
  const isAdmin = type === "admin";
  document.getElementById("login-type-label").textContent = isAdmin ? "Admin Login" : "User Login";
  document.getElementById("admin-demo").style.display = isAdmin ? "block" : "none";
  document.getElementById("user-demo").style.display = isAdmin ? "none" : "block";
  document.getElementById("login-email").placeholder = isAdmin ? "admin@transitops.com" : "driver@transitops.com";
...
function handleLogin() {
  ...
  const isAdminLogin = selectedLoginType === "admin";
  const userIsAdmin = ADMIN_ROLES.includes(user.role);
  if (isAdminLogin && !userIsAdmin) {
    alert("This account is not an admin account. Please use User Login.");
    return;
  }
  if (!isAdminLogin && userIsAdmin) {
    alert("Admin accounts must sign in via Admin Login.");
    return;
  }
...
function toggleTheme() {
  ...
  localStorage.setItem("to_dark_theme", document.body.classList.contains("dark-mode"));
}
...
function renderDashboard() {
  ...
  document.getElementById("kpi-total-vehicles").textContent = vehicles.filter(v => v.status !== "Retired").length;
  document.getElementById("kpi-total-drivers").textContent = drivers.length;
}
...
function renderVehicles() {
  ...
  list.forEach(v => {
    ...
    tr.innerHTML = `
      <td><img class="table-thumb" src="${v.imageUrl || DEFAULT_VEHICLE_IMAGE}" alt="${v.nameModel}"></td>
...
function editVehicle(id) {
  const v = window.db.getVehicles().find(item => item.id === id);
  if (!v) return;
  ...
  document.getElementById("vehicle-image").value = v.imageUrl || "";
...
function handleVehicleSubmit(e) {
  ...
  const payload = {
    ...
    imageUrl: document.getElementById("vehicle-image").value.trim()
  };
...
function renderDrivers() {
  ...
  list.forEach(d => {
    ...
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td><img class="table-avatar" src="${d.imageUrl || DEFAULT_AVATAR}" alt="${d.name}"></td>
...
function editDriver(id) {
  const d = window.db.getDrivers().find(item => item.id === id);
  if (!d) return;
  ...
  document.getElementById("driver-image").value = d.imageUrl || "";
...
function handleDriverSubmit(e) {
  ...
  const payload = {
    ...
    safetyScore: parseInt(document.getElementById("driver-safety").value, 10),
    status: document.getElementById("driver-status").value,
    imageUrl: document.getElementById("driver-image").value.trim()
  };
